...
def send_message(token: str, channel_id: str, message: str, log_fn):
    ...
    try:
        ...
        if response.ok:
            ...
            log_fn(f"[ERROR] Token: {token[:10]}... Status: {response.status_code}")
...
def main():
    st.set_page_config(page_title="cccpRaider (Python)", page_icon="🐍")
    st.title("cccpRaider")
    ...
    elif len(st.session_state.tokens) > token_count:
        st.session_state.tokens = st.session_state.tokens[:token_count]
